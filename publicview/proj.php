...
$sql= "SELECT Id_number, Opinion FROM project WHERE Id_number = ? AND(Opinion = 'YES' OR Opinion = 'NO')";
...
$opinion_err =  $_SESSION["Firstname"] . ", you had voted earlier. Please wait for the next event.";
}
else{
...
$sql = "UPDATE project SET Opinion = ?, Comment = ? WHERE Id_number = ?";

if($stmt = mysqli_prepare($con,$sql)){

mysqli_stmt_bind_param($stmt,"ssi", $param_opinion, $param_comment, $param_id);

$param_opinion = trim($_POST["opinion"]);
