...
$sql= "SELECT Id_number, Opinion FROM users WHERE Id_number = ? AND(Opinion = 'YES' OR Opinion = 'NO')";
...
$sq = "SELECT Id_number, Opinion FROM users WHERE Id_number = ? AND Opinion = ?";

if($stmt = mysqli_prepare($con,$sq)){
mysqli_stmt_bind_param($stmt, "is",$id,$opin);
...
if(mysqli_stmt_num_rows($stmt) == 1){
$opinion="";
$opinion_err = $_SESSION["Firstname"]. ",Please! No voting is ongoing currently, You will be notified incase of any.";
}
}else{
echo "Oops!. fail to execute sql query" . mysqli_error($con);
}
}
mysqli_stmt_close($stmt);
...
$sql = "UPDATE users SET Opinion = ?, Comment = ? WHERE Id_number = ?";
